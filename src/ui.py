...
def print_tool_call(tool_name: str, arg_summary: str):
    ...
    if name_lower in ["read_file", "read"]:
        ...
        console.print(f"\n[bold]● Narzędzie ({tool_name}): {arg_summary}[/bold]")
...
def print_code_panel(path: str, content: str, lexer_override: str = None):
    ...
    display_content = "\n".join(lines[:20])
    subtitle = f"[gray50]... (i {len(lines)-20} więcej linii)[/gray50]" if len(lines) > 20 else None
...
class ThinkingTree:
    def __init__(self, expanded=True, simulate=False):
    ...
    def render(self):
        ...
        t = Text()
        t.append(f"\n{sym} Thinking... ({format_time(elapsed_sec)})\n", style=ACCENT_COLOR)
        if self.expanded:
    ...
    def print_tree(self):
        ...
        elapsed_sec = time.time() - self.start_time
        console.print(f"\n[{ACCENT_COLOR}]✻ Thinking... ({format_time(elapsed_sec)})[/]")
        if self.expanded:
            for line in self.lines:
                spaces = len(line) - len(line.lstrip())
                indent = " " * spaces
...
def print_turn_done(elapsed: float, tokens: int, tool_count: int):
    ...
    console.print(f"\n[{ACCENT_COLOR}]✻ Done[/] ({format_time(elapsed)} · ⛁ {t_str} tokens · {tool_count} tool calls)")
class SearchSpinner:
    ...
    def stop(self, result_summary: str, details: str = ""):
        ...
        if details:
            ...
            if len(lines) > 5:
                console.print(f"    [{MUTED_COLOR}]|_ ... i {len(lines) - 5} więcej[/]")
